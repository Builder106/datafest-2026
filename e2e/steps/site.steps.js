...
Then('the adjusted odds ratio {string} is visible', async ({ page }, value) => {
  await expect(page.locator('.stat-num').first()).toContainText(value);
});

Then('the ED rate {string} is visible', async ({ page }, value) => {
...
Then('all {int} robustness check cards are present', async ({ page }, count) => {
